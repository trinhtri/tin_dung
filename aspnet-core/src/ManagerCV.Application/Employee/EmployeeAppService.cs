...
namespace ManagerCV.Employee
{
	public class EmployeeAppService : ManagerCVAppServiceBase, IEmployeeAppService
	{
		...
		public async Task<PagedResultDto<EmployeeListDto>> GetAll(EmployeeInputDto input)
		{
			...
			{
				start = new DateTime(input.StartDate.Value.Year, input.StartDate.Value.Month, input.StartDate.Value.Day, 0, 0, 1);
			}
			if (input.EndDate.HasValue)
			{
				end = new DateTime(input.EndDate.Value.Year, input.EndDate.Value.Month, input.EndDate.Value.Day, 23, 59, 59);
			}
			var query = _employeeRepository.GetAll()
					  ...
					  || x.NamSinh.ToUpper().Contains(input.Filter.ToUpper())
					  || x.SDT.ToUpper().Contains(input.Filter.ToUpper())
					  || x.KinhNghiem.ToUpper().Contains(input.Filter.ToUpper())
					  )
					   .WhereIf(input.StartDate.HasValue, x => x.CreationTime >= start)
					   .WhereIf(input.EndDate.HasValue, x => x.CreationTime <= end);
		...
		public async Task<PagedResultDto<EmployeeListDto>> GetCVByStatus(EmployeeGuiInputDto input)
		{
			...
			DateTime startNPV = DateTime.Now;
			DateTime endPV = DateTime.Now;
			if (input.StartDate.HasValue)
			{
				start = new DateTime(input.StartDate.Value.Year, input.StartDate.Value.Month, input.StartDate.Value.Day, 0, 0, 1);
			}
			...
			if (input.StartNgayPV.HasValue)
			{
				startNPV = new DateTime(input.StartNgayPV.Value.Year, input.StartNgayPV.Value.Month, input.StartNgayPV.Value.Day, 0, 0, 1);
			}
			if (input.EndNgayPV.HasValue)
			{
				endPV = new DateTime(input.EndNgayPV.Value.Year, input.EndNgayPV.Value.Month, input.EndNgayPV.Value.Day, 23, 59, 59);
			}
			...
			var query = (from sendCV in _sendCVRepository.GetAll()
					  ...
					  .WhereIf(!input.Filter.IsNullOrEmpty(),
						...
						.WhereIf(input.EndDate.HasValue, x => x.CreationTime <= end)
			            .WhereIf(input.StartNgayPV.HasValue, x => x.NgayPhongVan >= startNPV)
						.WhereIf(input.EndNgayPV.HasValue, x => x.NgayPhongVan <= endPV);
		...
		public async Task<FileDto> GetCVNewToExcel(EmployeeGuiInputDto input)
		{
			var list = await GetCVNew(input);
			var dto = list.Items.ToList();
			return _employeeListExcelExporter.Export(dto);
		}
		...
		{
			var list = await GetCVByStatus(inputDto);
			var dto = list.Items.ToList();
		...
		public async Task DaNhan(int id, DateTime? NgayDiLam, string note)
		{
			var sendCV = await _sendCVRepository.FirstOrDefaultAsync(id);
